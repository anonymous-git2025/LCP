...
def get_obstacle_info(position, heading, base_heading, sim):
    theta = (base_heading - heading)/2      
...
def edge_vec_to_indexes(edge_vec,base_heading,number=24):
    ''' angle index
        {0, 1, ..., 23} for 24 angles, 15 degrees separation
    '''


    angle = np.arctan2(1.0, 0.0) + np.arctan2(edge_vec[1], edge_vec[0]) + base_heading     
    while angle < 0.0:
        angle += 2 * math.pi
