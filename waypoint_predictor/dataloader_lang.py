...
class MyDataLoader(Dataset):
    def __init__(self, args, img_dir, navigability_dict):
        ...
        for img_dir in glob.glob(img_dir):
            ...
            if self.navigability_dict[scan_id][episode_id][int(step_id)] == None or step_id == 0:     
                self.img_dirs.remove(img_dir)      
    ...
    def __getitem__(self, idx):
        ...
        if self.lang == 'h_t':
            h_t = torch.from_numpy(rgb_depth_img['h_t'])
            sample['lang'] = h_t
        elif self.lang == 'lang_state_scores':
            lang_state_scores = torch.from_numpy(rgb_depth_img['lang_state_scores'])
            sample['lang'] = lang_state_scores
        elif self.lang == 'attened_lang':
            attened_lang = torch.from_numpy(rgb_depth_img['attened_lang'])
            sample['lang'] = attened_lang
