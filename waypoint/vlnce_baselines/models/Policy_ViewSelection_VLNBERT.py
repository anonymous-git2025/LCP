...
class VLNBERT(Net):
    def __init__(
        ...
        self.vln_bert.config.directions = 1 
    ...
    def forward(self, mode=None, 
        ...
        if mode == 'language':
            ...
            waypoint_output = []
            for idx in range(len(waypoint_predictor)):
                waypoint_output.append(waypoint_predictor[idx].get_cand(
                    source_info[idx]))    
         
            
            candidate_lengths = [len(cand_info['idx']) + 1 for cand_info in waypoint_output]
            if isinstance(candidate_lengths, int):
                candidate_lengths = [candidate_lengths]
            max_candidate = max(candidate_lengths) 
            cand_mask = length2mask(candidate_lengths, device=self.device)
            ...
            for j in range(batch_size):
                pred_idx = waypoint_output[j]['idx']
                pred_pos = waypoint_output[j]['pos']
                pred_ang = torch.tensor(waypoint_output[j]['ang'])
                pred_dis = torch.tensor(waypoint_output[j]['dis'])
                
             
                pred_ang = 2*math.pi-pred_ang.float()
                
                angle_rad_cc = 2*math.pi-pred_ang.float()
                batch_angles.append(angle_rad_cc.tolist())
                angle_rad_c = pred_ang.cpu().float()
                batch_angles_c.append(angle_rad_c.tolist())
                batch_distances.append(pred_dis.tolist())
               
                img_idxes = 12 - (pred_ang.cpu().numpy() * 12 + math.pi) // (2 * math.pi)
                img_idxes[img_idxes==12] = 0
                ...
                batch_idx.append(pred_idx)
                batch_pos.append(pred_pos)
          
            cand_direction = dir_angle_feature_with_ele(batch_angles_c).to(self.device)
            
            if in_train:
                return cand_rgb, cand_depth, cand_direction, cand_mask, candidate_lengths, batch_angles, batch_distances, batch_idx, batch_pos
            else:
                return cand_rgb, cand_depth, cand_direction, cand_mask, candidate_lengths, batch_angles, batch_distances, batch_idx, batch_pos
        
        elif mode == 'waypoint_lang':
            batch_size = observations['instruction'].size(0)
            ''' encoding rgb/depth at all directions ----------------------------- '''
            NUM_ANGLES = 120   
            NUM_IMGS = 12
            ...
            ''' waypoint prediction ----------------------------- '''
            waypoint_heatmap_logits = waypoint_predictor(
                rgb_embedding, depth_embedding, my_lang_feat)
